...
/// A 32‐byte array wrapper for the “raw” 32‐byte message hash.
/// On Ethereum, you’d normally hash your data (e.g. `keccak256(data)`) → 32 bytes.
/// That 32‐byte array is what we wrap in `EthHash`.
#[derive(Copy, Clone, PartialEq, Eq)]
pub struct EthHash(pub [u8; 32]);

/// A 65‐byte array wrapper for the “Ethereum‐style signature” in [r||s||v] format.
/// Where `r` = 32 bytes, `s` = 32 bytes, `v` = 1 byte (27 or 28).
#[derive(Copy, Clone, PartialEq, Eq)]
pub struct Signature(pub [u8; 65]);

/// A 20‐byte Ethereum address wrapper.  
/// It corresponds to the low‐20 bytes of keccak256(uncompressed_pubkey[1..]) on Ethereum.
...
impl EthSigner {
    ...
    /// Sign a 32‐byte “raw” hash using Ethereum‐style:
    /// 1. Prefix with `"\x19Ethereum Signed Message:\n32"`.
    /// 2. Keccak256 → 32 bytes.
    /// 3. sign_prehash (r||s), then compute v = recovery_id + 27.
    ...
    fn compute_recovery_id(&self, hash: &[u8; 32], sig: &k256Signature) -> u8 {
        let rec_id_0 = VerifyingKey::recover_from_prehash(hash, sig, RecoveryId::new(false, false));
        let rec_id_1 = VerifyingKey::recover_from_prehash(hash, sig, RecoveryId::new(true, false));
